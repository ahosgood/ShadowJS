		...
		var Shadow = function() {
					...
					if( elm.style.animationName ) {
						...
						for( var i = 0; i < domPrefixes.length; i++ ) {
							if( elm.style[ domPrefixes[i] + 'AnimationName' ] !== undefined ) {
								pfx = domPrefixes[i];
								strAnimation = pfx + 'Animation';
								strKeyframePrefix = '-' + pfx.toLowerCase() + '-';
								blAnimation = true;
								break;
							}
						}
			...
			this.padLeft = function( strRaw, intFinalLength, strPadChar ) {
					...
					if( strRaw.length < intFinalLength ) {
						for( var i = 0; i < intFinalLength; i++ ) {
			...
			this.prettySize = function( intBytes ) {
					...
					return this.round( intBytes / Math.pow( 1024, intLimit ), 2 ) + ' ' + objLimits[intLimit + 1];
				},
			...
			this.time = function( mxdFormatUTC, blUTC ) {
					...
					if( typeof strFormat === 'string' ) {
						...
						while( i < strFormat.length ) {
							var strChar = strFormat.charAt( i );

							if( strChar === '\\' ) {
								i++;
								strOut += strFormat.substr( i, 1 );
							} else if( strChar === 'U' ) {
								if( blUTC ) {
		...
		if( window.jQuery ) {
			( function( $ ) {
					$.fn.formToObject = function() {
							...
							/*function stringToObject( strIn, strValue ) {
								var mxdOut;

								//console.log( 'IN: ' + strIn + ' = ' + strValue );

								var intStart = strIn.indexOf( '[' );
								var intEnd = strIn.indexOf( ']' );

								//if has brackets and length is more than 0
								if( strIn.length > 0
										&& intStart !== -1
										&& intEnd !== -1 ) {
									//if there is something before first bracket
									if( intStart > 0 ) {
										//console.log( 'HIT 1' );
										//get stuff before the first bracket (this is the name)
										var strThisName = strIn.substring( 0, intStart );
										mxdOut = {};
										//process stuff after name - RECALL
										mxdOut[strThisName] = stringToObject( strIn.substring( intStart ), strValue );
									//else if there is some crap in the first brackets
									} else if( intEnd > 1 ) {
										//console.log( 'HIT 2' );
										//get the stuff in the brackets (this is the key)
										var strThisName = strIn.substring( 1, intEnd );
										mxdOut = {};
										//process stuff after key - RECALL
										mxdOut[strThisName] = stringToObject( strIn.substring( intEnd + 1 ), strValue );
									//else the first brackets are blank
									} else {
										//console.log( 'HIT 3' );
										//process stuff after first brackets - RECALL
										mxdOut = [stringToObject( strIn.substring( intEnd + 1 ), strValue )];
									}
								} else {
									//console.log( 'HIT 4' );
									//return in as name
									mxdOut = {};
									mxdOut[strIn] = strValue;
								}

								//console.log( 'OUT:', mxdOut );

								return mxdOut;
							}

							function arrayToObject( arrRaw ) {
								var objReturn = {};

								for( var mxdIndex in arrRaw ) {
									if( typeof mxdValue === 'array' ) {
										objReturn[mxdIndex] = arrayToObject( arrRaw[mxdIndex] );
									} else {
										objReturn[mxdIndex] = arrRaw[mxdIndex];
									}
								}
							...
							/*jQuery.map( $( this ).serializeArray(),
								function( jqoElement ) {
									...
									var intStart = jqoElement['name'].indexOf( '[' );
									var intEnd = jqoElement['name'].indexOf( ']', intStart + 1 );

									if( intStart !== -1
											&& intEnd !== -1 ) {
										var strThisName = jqoElement['name'].substring( 0, intStart );
										var strNextName = jqoElement['name'].substring( intStart + 1, intEnd );

										if( typeof objOut[strThisName] === 'undefined' ) {
											objOut[strThisName] = {};
										}

										objOut[strThisName][strNextName] = jqoElement['value'];
									} else {
										objOut[jqoElement['name']] = jqoElement['value'];
									}


									//var objInputName = arrayToObject( stringToObject( jqoElement['name'], jqoElement['value'] ) );
									//console.log( objInputName );
									//objOut = $.extend( objOut, objInputName );
