...
(
	function( window ) {
		var Shadow = function() {
				...
				this.cookieSet = function( strName, mxdData, intSecondsLife ) {
						if( typeof intSecondsLife === 'number' ) {
							var datNow = new Date();
							datNow.setTime( datNow.getTime() + ( 1000 * intSecondsLife ) );
							document.cookie = strName + '=' + mxdData +';expires=' + datNow.toGMTString() + ';path=/';
						} else {
					...
					$.fn.ShadowSwipe = function( objUserOptions ) {
							...
							return this.each(
								function() {
									...
									intSteepDiagonalLimit = 1 / intShallowDiagonalLimit,
									objOriginalPosition = { x: 0, y: 0 },
									objFinalPosition = { x: 0, y: 0 },
